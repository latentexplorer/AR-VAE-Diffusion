...
class DDPMWrapper(pl.LightningModule):
    def __init__(
        ...
        assert eval_mode in ["sample", "recons", "alsr", "variations_ddpm", "variations_vae"]
    ...
    def predict_step(self, batch, batch_idx, dataloader_idx=None):
        ...
        if self.eval_mode == "sample":
            x_t, z = batch
            z = z * self.normal_std + self.normal_mean
            recons = self.vae(z)
            recons = 2 * recons - 1
        ...
        elif self.eval_mode == "alsr":
            x_t, z = batch
            z = z * self.normal_std + self.normal_mean
            z[:,self.attribute_index]+=self.add_variation
            recons = self.vae(z)
            recons = 2 * recons - 1

        elif self.eval_mode == "variations_vae":
            x_t, z = batch
            #copy z, x_t for variations of same z, x_t
            z[:] = z[0:1]
            x_t[:] = x_t[0:1]

            z = z * self.normal_std + self.normal_mean
            z = z + 0.25 * torch.randn_like(z)
            recons = self.vae(z)
            recons = 2 * recons - 1
        ...
        elif self.eval_mode == "variations_ddpm":
            x_t, z = batch
            z[:] = z[0:1]
            x_t[:] = x_t[0:1]
            z = z * self.normal_std + self.normal_mean
            ...
            # Initial temperature scaling
            print(x_t.shape)
            x_t = x_t + 0.25*torch.randn_like(x_t)
            x_t = x_t * self.temp
